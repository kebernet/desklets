...
public class Registry {
        ...
        try {
            if(!Environment.HOME.exists()) {
                if(!Environment.HOME.mkdirs()) {
                    throw new Exception(
                            "Unable to make org.glossitope configuration directory.");
                }
            }
            ...
            if(!Environment.REPO.exists()) {
                if(!Environment.REPO.mkdirs()) {
                    throw new Exception(
                            "Unable to make org.glossitope repository directory.");
                }
            }
    ...
    public DeskletConfig installDesklet(String uuid, URL installFrom)
    throws MalformedURLException, IOException, JDOMException {
        SecurityManager sm = System.getSecurityManager();
        Object ctx = sm.getSecurityContext();
        sm.checkPermission(PERMISSION, ctx);
